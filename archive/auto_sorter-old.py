...
from utils import OBSIDIAN_ROOT
...
folders = [
    path for path in Path(OBSIDIAN_ROOT).glob('[!".", 0]*/**') if Path.is_dir(path)
]
...
files = [
    path
    for path in Path(OBSIDIAN_ROOT).rglob("*.md")
    if valid(path) and frontmatter.check(path)
]
...
for file in files:
    if "type" in frontmatter.load(file).keys():
        ...
        if note["type"] in tags:
            tag = tags[note["type"]]
        # sets default location if later doesn't match any if statements
    ...
    if tag in [
        "date-created",
        "year",
    ]:  # checks if file is a daily/quarterly/weekly entry
        year = month = ""
        ...
        if tag == "date-created":
            ...
            destination = (
                personal_destination / "Daily" / year / month
            )  # if month value then daily entry
        else:
        ...
        "monthly",
        "yearly",
    ]:  # sorts all other periodic entries
