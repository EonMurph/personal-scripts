from sqlalchemy import ForeignKey, Table, Column, String, Integer, Boolean
...
class Movie(Base):
    ...
    genres = relationship(
        ...
        cascade="merge",
    )  # creates relationship between genre and movie

    def __repr__(self):
        return (
            f"Movie: {self.name} ({self.year}) {self.genres}. Watched: {self.watched}"
        )
...
class Show(Base):
    ...
    genres = relationship(
        "Genre",
        secondary=show_genre_association_table,
        back_populates="shows",
        cascade="merge",
    )  # creates relationship between show and genre
...
def add_movie(data, session):
    ...
    for genre in data["genres"]:
        movie.genres.append(session.query(Genre).filter(Genre.name == genre).first())
...
def add_show(data, session):
    ...
    for genre in data["genres"]:
        show.genres.append(session.query(Genre).filter(Genre.name == genre).first())
...
def update_show(data, session):
    ...
    session.query(Show).filter(Show.name == name, Show.year == year).update(
        update_values
    )
...
def update_movie(data, session):
    ...
    session.query(Movie).filter(Movie.name == name, Movie.year == year).update(
        update_values
    )
...
def get_media(session, class_type, genre=None):
    list_of_media = [media for media in session.query(str_to_class(class_type)).all()]
    list_of_valid_media = [media for media in list_of_media if media.watched is False]
    if genre is not None:
        media_with_genre = [
