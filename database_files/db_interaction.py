...
action_type_group.add_argument(
    "--add",
    type=str,
    nargs="*",
...
for genre in list_of_genres:
    if (query := session.query(Genre).filter(Genre.name == genre)).first() is None:
...
if args.add is not None:
    ...
    if args.delete[0] == "Show":
        session.query(Show).filter(Show.name == name, Show.year == year).delete()
    if args.delete[0] == "Movie":
        session.query(Movie).filter(Movie.name == name, Movie.year == year).delete()
